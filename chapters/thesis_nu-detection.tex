...
\begin{table}[htb]
	\begin{minipage}{\textwidth}
		...
		The last column gives the expected total number of neutrino interactions per tonne of \ce{Ar} in the ND, excluding rock events.
		...
		\begin{tabu} to \textwidth {|c|S|S|S|S|S|}
			\hline
			Phase &			{$E_{\Pp} \ \qty[\si{\giga\electronvolt}]$} &	{\Pp per pulse} &	{Pulse period $\qty[\si{\second}]$} &	{Power $\qty[\si{\mega\watt}]$} &	{ND \si{evt\per\tonne} \ce{Ar}} \\
			\hline
