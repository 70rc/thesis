\chapter{Research and development of novel LArTPC detectors\label{chap:rd-lartpc}}

\section{TPC\label{sec:rd-lartcp_tpc}}

\section{Studies of the electric breakdown in liquid argon\label{sec:rd-lartpc_hv}}
...
\subsection{Setup of the HV tests\label{sec:rd-lartpc_hv_setup}}
\begin{figure}[htb]
	\centering	
	...
	\label{fig:rd-lartpc_hv_setup1}
\end{figure}

The setup used in this study is very similar to the one described in~\cite{breakdown_14} and is shown in Figure~\ref{fig:rd-lartpc_hv_setup1}.
...
\subsection{Experimental results\label{sec:rd-lartpc_hv_results}}
...
In our earlier measurements~\cite{breakdown_14}, we experienced sporadic discharges across the ribs of the dielectric of the high voltage feed-through.
In the present study, we managed to suppress these discharges completely by rising the level of the liquid argon by about \SI{20}{\centi\metre}.
...
\begin{table}[htb]
	\centering\caption{Summary of the measurement runs.}
	\label{tab:hv_table2}
	\begin{tabular}{|S|S|l|l|S|S|S|}
		\hline
		{{Run}} &	$\oslash_{\m{Sphere}}$ &	Surface finish &	Sphere pol. &	{{Events}} &	{{$d_{\m{Gap}}$}} &					{{$V_{\m{Breakdown}}$}} \\
		\hline
...
In Figure~\ref{fig:hv_iv}, we show the current and voltage features of a similar breakdown from a \SI{4}{\centi\metre} diameter cathode at \SI{6.0}{\milli\metre} from the anode plate.
...
\subsection{Interpretation of the results\label{sec:rd-lartpc_hv_interpretation}}
...
In Figure \ref{fig:hv_powerplot}, we combine data available in literature with those obtained from this work.
...
The electro-luminescence spectrum we measured (blue curve in Figure~\ref{fig:hv_spectro}) exhibits a broad continuum, similar to the scintillation spectrum.
...
However, if attributable to oxygen, this line has to also be observed at the later stages of the discharge, which does not take place in our measurements.
...
The presence of an observable emission at about 580~nm, in this case, is inevitably linked to a presence of ionized states.
...
To calculate the emission intensity we summed up the raw values of all pixels of the camera image that were in a row perpendicular to the cone direction.
The distance from the cathode can be derived from the known gap distance.
From the fit we obtained the given statistical uncertainty of $\alpha$.
...
As we already suggested in~\cite{breakdown_14}, positive ions produced in this process drift towards the cathode, raising the surface field and provoking a rapid increase of the field emission current to values of the order of several \si{\milli\ampere}.
...
In the region behind the head of the streamer the filament is collapsed to a diameter below \SI{200}{\micro\metre} (the spatial resolution of our camera) by surface tension and electro-striction forces.
...
This behavior is also suggested by the two spectra in Figure~\ref{fig:hv_spectro}.
...
\begin{figure}[p]
	\centering
	\includegraphics[height=0.43\textheight]{hv/allGood_IPeakVsE}
...
\begin{figure}[p]
	\centering
	\includegraphics[height=.43\textheight]{hv/selected_IPlateauVsE}
...
\begin{figure}[p]
	\centering
	\includegraphics[height=.43\textheight]{hv/selected_IPlateauVsIPeak}
...
\begin{figure}[p]
	\centering
	\includegraphics[height=.43\textheight]{hv/selected_vStreamerVsE}
...
In Figures~\ref{fig:hv_chargeVsVset} to~\ref{fig:hv_vStreamerVsE}, we show several correlations of measured and calculated parameters of the breakdowns.
...
The discrepancy to the total number of events in Table~\ref{tab:hv_table2} arises, on the one hand, because we installed the shunt resistor only in the last run and, on the other hand, since the latter was damaged after the events shown in the bottom plot of Figure~\ref{fig:hv_chargeVsVset}.
...
This also explains the behavior of the charge in Figure~\ref{fig:hv_chargeVsVset}.
...
The good match between the red curve and the data points serves also as a crosscheck of the tuned capacitance.
...
The knowledge of the streamer velocity can be applied in the design of protection circuits for future LArTPCs.
...
\section{A method to suppress electric breakdowns in liquid argon\label{sec:rd-lartpc_latex}}
Besides the thorough characterisation of breakdowns in LAr, we have found a way to suppress them by coating the field cage with latex\cite{latex}.
We were able to increase the voltage by a factor of \num{10} using this technique.


\section{A more robust approach to TPC readout wires\label{sec:rd-lartpc_cuoka}}
As outlined in Section~\ref{sec:lartpc_challenges}, one of the big challenges for future LArTPCs will be the mechanical stability of the classical wire readout planes for the envisioned detector sizes.
...
To investigate this solution, a proof of concept was performed at LHEP at the Univeristy of Bern.

\subsection{Experimental setup\label{sec:rd-lartpc_cuoka_setup}}
...
Section~\ref{sec:rd-dune-nd_electronics} describes the readout chain in more detail.

\subsection{Experimental results\label{sec:rd-lartpc_cuoka_results}}
...
Because of this, it was decided not to further investigate copper on Kapton readouts and instead focus on pixelated readouts for LArTPCs providing real 3D data.