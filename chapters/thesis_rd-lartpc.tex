...
Besides the thorough characterisation of breakdowns in LAr, a way was found to suppress them by coating the field cage with latex\cite{latex}.
It was possible to increase the voltage by a factor of \num{10} using this technique.

The setup was the same as the one used to study the breakdowns, described in Section~\ref{sec:rd-lartpc_hv}.
...
Its dielectric strength is reported to be in the range of \SIrange{1}{2}{\mega\volt\per\centi\metre}~\cite{sauce}, its dielectric constant is \num{2.1} which is close to the \num{1.6} of liquid argon, and its room temperature resistivity is \SI{1e16}{\ohm\centi\metre}.
A polyisoprene layer of several \SI{100}{\micro\metre} can be doposited on the sphere by dipping the latter in purified latex milk.
After drying at room temperature, the coating is leached in dionised water for several hours and finally vulcanised at \SI{70}{\celsius} for one hour.
Leaching is needed to remove all soluable pollutants contained in natural latex while the vulcanisation increases the tear strength of the coating.
