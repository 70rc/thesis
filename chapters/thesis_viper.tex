...
Therefore, the charge readout electronics used in \pixlar{} are quite similar to the chain described in Section~\ref{sec:electronics_AT} after the upgrades.
To trigger on scintillation light, one end of the TPC is equipped with a \SI{43 x 15}{\centi\metre} \AL{} module (see Chapter~\ref{chap:light-ro}), while the other end features an ARAPUCA~\cite{arapuca} detector for comparison.
