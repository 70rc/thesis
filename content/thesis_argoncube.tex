...
\AC{} is made of fully self-contained \gls{ac:tpc} modules.
...
Another big problem that can be solved by a modular \gls{ac:tpc} design are the high electric fields required for large detectors.
Because each module contains its own \gls{ac:tpc} independent of all other modules, the required cathode potential only depends on the module size not the detector size.
...
However, a modular \gls{ac:tpc} introduces a new challenge: The dead spaces in between adjacent \glspl{ac:tpc} have to be kept to a minimum because they introduce gaps in the recorded event topologies accompanied by lost energy.
