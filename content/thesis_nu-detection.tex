...
The detector consisted of a tank filled with \SI{615}{\tonne} of tetrachloroethylene, \ce{C_{2}Cl_{4}}.
...
In 1988, the \gls{kamiokande}, in the Kamioka mine in Japan, and the \gls{imb} detector~\cite{imb}, in a Morton Salt mine in Ohio, USA, found a similar deficiency in atmospheric neutrinos.
...
Its use of heavy water (\ce{D_{2}O}) allowed it to detect neutrinos flavour-independently via
\begin{IEEEeqnarray}{Cl}
...
For this to work, the emerging neutron needs to be detected which was achieved using \ce{^{3}He}-filled proportional counters inside the heavy water tank.
...
The spatial resolution of emulsions is limited by crystal size.
...
\begin{table}[tbp]
	\begin{minipage}{\textwidth}
		...
		\caption[\glsentryshort{dune} beam parameters and \glsentryshort{nd} rates]{%
			...
			The last column gives the expected total number of neutrino interactions per tonne of argon and beam spill in the \acrshort{nd}.
			...
			Note that these values are slightly different from the ones in Table~\ref{tab:nu-detection_nd-rates} because the latter are outdated.
			In accordance with \cite{dune2} most calculations in this work assume the \SI{2}{\mega\watt} \SI{80}{\giga\electronvolt} beam, i.e.\ \num{0.2}~events per tonne of argon and beam spill.
...
Figure~\ref{fig:nu-detection_dune-osc} shows the (anti)neutrino oscillation probability as a function of neutrino energy at the \dune{} baseline for normal and inverted mass ordering.
...
\begin{figure}[tbp]
	\centering
	\includegraphics[width=.495\textwidth]{dune/mh_exp_syst}
	\includegraphics[width=.495\textwidth]{dune/cpv75_exp_syst}
	\caption[\glsentryshort{dune} sensitivity]{%
...
\begin{table}[tbp]
	...
	\caption[\glsentryshort{dune} \glsentryshort{nd} event rates]{%
		Estimated number of interactions per tonne of argon in the \acrshort{dune} \acrshort{nd} for approximately one month (\num{1e20}~\acrshort{pot}) exposure to an (anti)neutrino beam produced from a primary proton beam of \SI{120}{\giga\electronvolt} and \SI{1.2}{\mega\watt}.
...
A brief explanation of the cross-section concept is given here to better understand the meaning of Figure~\ref{fig:nu-detection_xsec_flux}.
...
\begin{description}
	...
	\item[$I$] is the mean excitation energy of the absorber material, which can be approximated by
		\begin{IEEEeqnarray}{rCl}
			I & = & 16 Z ^ {0.9} \si{\electronvolt} \qq{for} Z > 1 \qq*{, and}
		\end{IEEEeqnarray}
...
\begin{figure}[tbp]
	...
	\caption[Stopping power]{%
		Bethe-Bloch mass stopping power of \Pgm in \ce{Ar}.
	}
...
Particles in this momentum range are called \emph{\glspl{mip}}.
...
\begin{IEEEeqnarray}{rCl}
	- \frac{1}{\rho}\dv{E}{x} & = & \frac{E}{X_{\m{0}}} \qc
...
At $E_{\Pgg} \ge 2 m_{\Pe} c ^ 2$, photons are capable of producing pairs of \Pep and \Pem.
...
\begin{IEEEeqnarray}{rCl}
	R_{0.95}^{\m{had}} & \leq & \lambda_{\m{int}} \qc
