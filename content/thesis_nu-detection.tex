...
The masses of relativistic neutrinos can be neglected, replacing momentum by energy ($p \approx E$) and time by baseline ($t = ct \approx L$).
...
First, the same result can be obtained by replacing $- i E_i t$ with $i \va{p_i} \va{x}$ and assuming equal energy instead of momentum for all mass eigenstates.
Second, both assumptions are wrong from a physics point of view.
Furthermore, they both assume a plane wave which is also wrong.
...
Both points are proved and illustrated in~\cite{Akhmedov}.
