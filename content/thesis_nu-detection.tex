...
In 1988, the \gls{kamiokande}, in the Kamkioka mine in Japan, and the \gls{imb} detector~\cite{imb}, in a Morton Salt mine in Ohio, USA, found a similar deficiency in atmospheric neutrinos which actually were a background for the original experiments looking for proton decays.
...
\begin{table}[htb]
	\begin{minipage}{\textwidth}
		...
		\caption[\glsentryshort{dune} beam parameters and \glsentryshort{nd} rates]{
			Summary of the \dune{} proton beam parameters for various configurations.
			...
			The last column gives the expected total number of neutrino interactions per tonne of \ce{Ar} in the \gls{nd}, excluding rock events.
			It is calculated by multiplying the expected neutrino flux with the cross-section on \ce{Ar} from the GENIE\footnote{\url{https://genie.hepforge.org}} neutrino event generator.
		...
		\begin{tabu} to \textwidth {cSSSSS}
			\toprule
			Phase &			{$E_{\Pp} \ \qty[\si{\giga\electronvolt}]$} &	{\Pp per pulse} &	{Pulse period $\qty[\si{\second}]$} &	{Power $\qty[\si{\mega\watt}]$} &	{\gls{nd} \si{evt\per\tonne} \ce{Ar}} \\
			\midrule
...
\begin{figure}[htb]
	...
	\caption[\glsentryshort{dune} neutrino fluxes.]{
		Neutrino fluxes for the \dune{} reference beam design operating in neutrino mode (left) and antineutrino mode (right), generated with a \SI{120}{\giga\electronvolt} primary proton beam.~\cite{dune2}
	}
...
\begin{table}[htb]
	...
	\caption[\glsentryshort{dune} \glsentryshort{nd} event rates]{
		Estimated number of interactions per tonne of \ce{Ar} at the \dune{} \gls{nd} for approximately one month (\num{1e20}~\gls{pot}) exposure to an (anti)neutrino beam produced from a primary proton beam of \SI{120}{\giga\electronvolt} and \SI{1.2}{\mega\watt}, taken from~\cite{dune2}.
	...
	\begin{tabu} to \textwidth {llSS}
		...
		\midrule
		\gls{cc} \gls{qe} &			\HepProcess{\Pgngm\Pn \to \Pgmm\Pp} &																			30000 &				13000 \\
		\gls{nc} elastic &			\HepProcess{\Pgngm\nucleon \to \Pgngm\nucleon} & 																11000 &				6700 \\
		\gls{cc} \gls{res} &		\HepProcess{\Pgngm\Pp \to \Pgmm\Pp\Pgpp} &																		21000 &				0 \\
		\gls{cc} \gls{res} &		\HepProcess{\Pgngm\Pn \to \Pgmm\Pn\Pgpp\, (\Pp\Pgpz)} &															23000 &				0 \\
		\gls{cc} \gls{res} &		\HepProcess{\Pagngm\Pp \to \Pgmp\Pp\Pgpm\, (\Pn\Pgpz)} &														0 &					8300 \\
		\gls{cc} \gls{res} &		\HepProcess{\Pagngm\Pn \to \Pgmp\Pn\Pgpm} &																		0 &					12000 \\
		\gls{nc} \gls{res} &		\HepProcess{\Pgngm\Pp \to \Pgngm\Pp\Pgpz\, (\Pn\Pgpp)} &														7000 &				0 \\
		\gls{nc} \gls{res} &		\HepProcess{\Pgngm\Pn \to \Pgngm\Pn\Pgpp\, (\Pp\Pgpz)} &														9000 &				0 \\
		\gls{nc} \gls{res} &		\HepProcess{\Pagngm\Pp \to \Pagngm\Pp\Pgpm\, (\Pn\Pgpz)} &														0 &					3900 \\
		\gls{nc} \gls{res} &		\HepProcess{\Pagngm\Pn \to \Pagngm\Pn\Pgpm} &																	0 &					4700 \\
		\gls{cc} \gls{dis} &		\HepProcess{\Pgngm\nucleon \to \Pgmm\particles} or \HepProcess{\Pagngm\nucleon \to \Pgmp\particles} &			95000 &				24000 \\
		\gls{nc} \gls{dis} &		\HepProcess{\Pgngm\nucleon \to \Pgngm\particles} or \HepProcess{\Pagngm\nucleon \to \Pagngm\particles} &		31000 &				10000 \\
		\gls{cc} \gls{coh} \Pgpp &	\HepProcess{\Pgngm\nucleus \to \Pgmm\nucleus\Pgpp} &															930 &				0 \\
		\gls{cc} \gls{coh} \Pgpm &	\HepProcess{\Pagngm\nucleus \to \Pgmp\nucleus\Pgpm} &															0 &					800 \\
		\gls{nc} \gls{coh} \Pgpz &	\HepProcess{\Pgngm\nucleus \to \Pgngm\nucleus\Pgpz} or \HepProcess{\Pagngm\nucleus \to \Pagngm\nucleus\Pgpz} &	520 &				450 \\
		\gls{nc} elastic electron &	\HepProcess{\Pgngm\Pem \to \Pgngm\Pem} or \HepProcess{\Pagngm\Pem \to \Pagngm\Pem} &							16 &				11 \\
		Inverse muon decay &		\HepProcess{\Pgngm\Pem \to \Pgmm\Pgne} &																		9.5 &				0 \\
		\midrule
		Total \gls{cc} &			&																												170000 &			59000 \\
		Total \gls{nc}+\gls{cc} &	&																												230000 &			84000 \\
		\bottomrule
	\end{tabu}
\end{table}
...
\begin{figure}[htb]
	...
	\caption[Neutrino electron scattering]{
		\gls{nc} (left) and \gls{cc} (right) neutrino electron scattering.
	}
...
\begin{figure}[htb]
	...
	\caption[Antineutrino electron scattering]{
		\gls{nc} (left) and \gls{cc} (right) antineutrino electron scattering.
	}
...
\begin{figure}[htb]
	...
	\caption[Neutrino interaction cross-section]{
		...
		The cross-section is split into contributions from \gls{cc} and \gls{nc} interactions.
		For \gls{cc}, the individual contribution from \gls{res} interactions is shown, as well as from the sum of 1p1h and 2p2h. The latter two correspond to the \gls{qe} channel and interactions involving \glspl{mec}, respectively.
	}
	\todo[inline]{sauce}
