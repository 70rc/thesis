...
\begin{table}[htb]
	\begin{minipage}{\textwidth}
		...
		\caption[\glsentryshort{dune} beam parameters and \glsentryshort{nd} rates]{
			...
			The last column gives the expected total number of neutrino interactions per tonne of \ce{Ar} in the \acrshort{nd}, excluding rock events.
		...
		\begin{tabu} to \textwidth {cSSSSS}
			\toprule
			Phase &			{$E_{\Pp} \ \qty[\si{\giga\electronvolt}]$} &	{\Pp per spill} &	{Spill period $\qty[\si{\second}]$} &	{Power $\qty[\si{\mega\watt}]$} &	{\acrshort{nd} \si{evt\per\tonne_{\ce{Ar}}}} \\
			\midrule
...
The aforementioned \si{\mega\watt} beam intensity leads to \gls{nd} event rates $\sim{0.1}$ events per tonne of \ce{Ar} leading to significant event pile-up (see Table~\ref{tab:nu-detection_beam-params}).
...
\begin{figure}[htb]
	...
	\caption[Neutrino interaction cross-section]{
		...
		For \acrshort{cc}, the individual contribution from \acrshort{res} interactions is shown, as well as from the sum of 1p1h and 2p2h. The latter two correspond to the \acrshort{qe} channel and interactions involving \acrshortpl{mec}, respectively.
	}
	\todo[inline]{sauce}
