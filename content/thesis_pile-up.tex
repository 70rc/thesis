\section{Feasibility Study of a Pixelated \lartpc{} for the \dune{} Near Detector}
...
More details on \dune{} can be found in Section~\ref{sec:nu-detection_dune} while the proposed \AC{} \lar{} component of the ND is described in Section~\ref{sec:dune-nd_ac}.
...
\subsection*{\Pgpz pile-up simulation}
...
Section~\ref{sec:ac_viper} proves that this is feasible provided the current reconstruction ambiguities can eliminated by a successful deployment of the \larpix{} charge readout electronics described in Section~\ref{sec:studies_electronics}.
The spatial resolution of the pixel readout is assumed to be \SI{0.3}{\centi\metre} based on the ND design specified in Section~\ref{sec:dune-nd_ac}\todo{justify!}.
...
MicroBooNE has achieved a resolution in drift direction $< \SI{0.3}{\centi\metre}$~\cite{uboone}, making it safe to assume \larpix{} will have a similar performance, even though not yet fully characterised.
...
Active volume dimensions are taken from the preliminary \dune{} near detector design described in Section~\ref{sec:dune-nd_ac}.
...
The hadron containment studies described in Section~\ref{sec:dune-nd_ac} indicated that \SI{2.5}{\metre} height is the bare minimum.
