...
\subsection{\gls{ac:tpc}}
\label{sec:ac_viper_tpc}

The pixel demonstration \gls{ac:tpc}, shown in Figures~\ref{fig:viper_cad}~and~\ref{fig:viper_v1per}, is cylindrical with an inner diameter of \SI{101}{\milli\metre} and a \SI{590}{\milli\metre} drift length. 
The \gls{ac:tpc} operated with a drift field of \SI{1}{\kilo\volt\per\centi\metre}, corresponding to a total drift time of \SI{281}{\micro\second} at \SI{2.1}{\milli\metre\per\micro\second}.~\cite{protoLASER}

\begin{figure}[htb]
	\centering
	\includegraphics[width=0.9\linewidth]{viper/tpc_cad}
	\caption{\small Engineering drawing of the pixel demonstration \gls{ac:tpc}; \SI{590}{\milli\metre} drift length; \SI{6.25}{\milli\metre} field cage spacing; \SI{101}{\milli\metre} internal diameter.}
	\label{fig:viper_cad}
\end{figure}
...
Alternating acrylic rings are split, to allow for the circulation of purified \lar{} within the \gls{ac:tpc} volume.
...
\begin{figure}[htb]
	...
	\caption{\small  Left: Photograph of the  pixel demonstration \gls{ac:tpc} at Bern, with the HV feedthrough.
...
The pixel demonstration \gls{ac:tpc} is housed in a double-bath vacuum-insulated cryostat with the outer bath open to atmosphere.
...
To assess the \gls{ac:snr}, dedicated noise data was taken employing a \SI{5}{\hertz} random trigger.
...
\begin{figure}[htb]
	...
	The \gls{ac:tpc} volume is shown in faint grey.
	The passing particle is most likely a cosmic \Pgm entering from the left (the same event as in Figures~\ref{fig:viper_unfilteredRawData}~through~\ref{fig:viper_kalman}).
	Drift direction is from right to left.
...
It houses a \gls{ac:tpc} with \SI{47}{\centi\metre} drift length and a \SI{40 x 90}{\centi\metre} readout plane parallel to the beam direction, resulting in an active volume of \SI{170}{\litre}.
...
To trigger on scintillation light, one end of the \gls{ac:tpc} is equipped with a \SI{43 x 15}{\centi\metre} \AL{} module (see Section~\ref{sec:studies_light-col_al}), while the other end features an ARAPUCA~\cite{arapuca} detector for comparison.
